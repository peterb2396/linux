...
void* handle_client(void* arg) {
    ...
    while (1) {
        ...
        if (helper_pid == 0) // Child: Spawn helper node
        {
            // Make string versions of the pipe id's to pass to argv
            char helper_read[10]; // Buffer for converting arg1 to a string
            char helper_write[10]; // Buffer for converting arg2 to a string

            // Convert integers to strings
            snprintf(helper_read, sizeof(helper_read), "%d", helper_pipe[0]);
            snprintf(helper_write, sizeof(helper_write), "%d", helper_pipe[1]);
            
            // Child process: Call encode then die
            execl("helper", "helper", helper_read, helper_write, NULL);
            perror("execl");  // If execl fails
            exit(EXIT_FAILURE);
        }
        else // Parent: Obtain & decode result from helper
        {
            
            write(helper_pipe[1], buffer, strlen(buffer));
            close(helper_pipe[1]);  // Done writing frame to be decoded

            // When child is done, read result
            waitpid(helper_pid, NULL, 0);;

            // Parent reads result from the child process (the capped frame, encoded)
            char helper_result[strlen(buffer)];
            bzero(helper_result, sizeof(helper_result));

            // Listen for & store result from helper (same length as input, just capped)
            read(helper_pipe[0], helper_result, sizeof(helper_result));
            close(helper_pipe[0]);  // Done reading data from helper. Proceed as usual


            // Add <MSG> tags to the encoded message
            // Send the capitalized data to client, helper is done with it
            char tagged_frame[strlen(helper_result) + strlen("<MSG></MSG>")];
            bzero(tagged_frame, sizeof(tagged_frame));
            sprintf(tagged_frame, "<MSG>%s</MSG>", helper_result);
      ...
      // Helper result has been sent, and then decoded & stored.
