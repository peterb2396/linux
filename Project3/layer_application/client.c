...
void sendMessage(int message_len_l)
{
    ...
    int r_frame = malform? rand() % frames: -1; // Malform a random frame
...
void receiveMessages(int pipefd[2]) {
    ...
    while (1) {
        ...
        if (chatting && strlen(buffer2) > 800 && ((strstr(buffer2, "</MSG><MSG>")) != NULL))
        {
            //printf("GOT A BAD FRAME: %ld\nFRAME: %s", bytesRead, buffer2);
            const char *start = buffer2;
            while ((start = strstr(start, "<MSG>")) != NULL) {
                const char *end = strstr(start, "</MSG>");
                if (end == NULL) {
                    break;  // No closing </MSG> tag found, exit loop
                }

                // Calculate the length of content between <MSG> and </MSG>
                size_t contentLength = end - (start + 5);

                // Allocate a buffer to store the content and copy it
                char contentBuffer[contentLength + 1];
                strncpy(contentBuffer, start + 5, contentLength);
...
void receiveMessage(const char *content) {
    ...
    else // Parent
    {
        ...
        write(decode_pipe[1], content, strlen(content));
